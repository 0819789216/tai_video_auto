...
print(f"📌 Tìm thấy {len(urls)} link trong file {INPUT_FILE}. Bắt đầu tải...\n")

# ==========================================
# 2. CÁC HÀM TẢI (THREADS, COLLAB, STORYFUL, GDRIVE, IG, TIKTOK, DOUYIN)
# ==========================================
def download_threads_api(url, save_path):
    ...
    clean_url = re.sub(r"/media/?(?:\?.*)?$", "", clean_url, flags=re.IGNORECASE)
    ...
    for attempt in range(3):
        try:
            ...
            video_urls = [v.get("download_url") or v.get("url") for v in res.get("video_urls", []) if v]
...
def download_collab_inc_web(url, save_path):
    headers = {"User-Agent": "Mozilla/5.0", "Referer": "https://vl.collab.inc/", "Origin": "https://vl.collab.inc"}
    try:
        scraper = cloudscraper.create_scraper()
        res = scraper.get(url, headers=headers, timeout=15)
        video_matches = re.findall(r'https?://[^\s"\']+\.(?:mp4|m3u8)[^\s"\']*', res.text)
        for media_url in video_matches:
            media_url = media_url.replace("\\/", "/").replace("&amp;", "&")
            if ".mp4" in media_url and "m3u8" not in media_url:
                v_res = scraper.get(media_url, headers=headers, stream=True, timeout=25)
    ...
    if record_id:
        try:
            ...
            for media in res.get("story", {}).get("media", []):
                ...
                if video_url:
                    v_res = scraper.get(video_url, headers=headers, stream=True, timeout=25)
                    if v_res.status_code == 200:
                        with open(save_path, "wb") as f:
                            for chunk in v_res.iter_content(chunk_size=1024 * 1024):
                                if chunk: f.write(chunk)
...
def download_gdrive_api(url, save_path):
    try:
        ...
        if os.path.exists(save_path) and os.path.getsize(save_path) > 30000: return True
    except Exception:
        pass
    return False
...
def download_instagram_api(url, save_path):
    ...
    for attempt in range(3):
        try:
            scraper = cloudscraper.create_scraper()
            res = scraper.post("https://indown.io/fetch", data={"link": clean_url, "referer": "https://indown.io/"}, headers=headers, timeout=12)
            v_urls = re.findall(r'href="(https?://[^\s"]+\.mp4[^\s"]*)"', res.text)
            if v_urls:
                v_data = scraper.get(v_urls[0].replace("&amp;", "&"), timeout=25).content
                if len(v_data) > 100000:
                    with open(save_path, "wb") as f: f.write(v_data)
                    return True
        except Exception:
            time.sleep(1)
    return False
    ...
    for attempt in range(3):
        try:
            ...
            if res.get("code") == 0 and res.get("data"):
                ...
                if not v_url.startswith("http"): v_url = "https://www.tikwm.com" + v_url
...
def download_douyin_fast(url, save_path):
    cmd = ["yt-dlp", url, "-o", save_path, "--no-playlist", "--no-progress", "--quiet", "--no-warnings", "--referer", "https://www.douyin.com/", "--concurrent-fragments", "8"]
    if os.path.exists(COOKIES_FILE): cmd.extend(["--cookies", COOKIES_FILE])
    try:
        res = subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if res.returncode == 0 and os.path.exists(save_path) and os.path.getsize(save_path) > 30000: return True
    except Exception:
        pass
    return False

# ==========================================
# 3. VÒNG LẶP ĐIỀU PHỐI TẢI TUẦN TỰ
# ==========================================
for index, url in enumerate(urls, start=1):
    ...
    if not success:
        ydl_opts = {
            ...
            "no_warnings": True,
            "noprogress": True,
            "logger": SilentLogger(), # Chặn tuyệt đối log dư thừa
            "retries": 5,
