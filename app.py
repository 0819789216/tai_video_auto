    ...
    if not urls_input.strip():
        ...
        for line in iter(process.stdout.readline, ""):
            ...
            if "📌 Tìm thấy" in clean_line:
                try:
                    st.session_state.total_urls = int(clean_line.split("Tìm thấy ")[1].split(" link")[0])
                except Exception:
                    pass
        ...
        if os.path.exists(output_dir) and os.listdir(output_dir):
            ...
            # Tính chính xác số lượng link tải thành công
            failed_count = len(st.session_state.failed_links)
            if st.session_state.total_urls > 0:
                st.session_state.success_count = st.session_state.total_urls - failed_count
            else:
...
if st.session_state.zip_bytes is not None:
    ...
    file_count_text = f" (đã đóng gói vào ZIP tổng cộng {len(st.session_state.failed_links) + st.session_state.success_count} file MP4)" if st.session_state.success_count > 0 else ""
    st.success(f"🎉 Hoàn tất! Đã tải thành công {st.session_state.success_count}/{st.session_state.total_urls} link video.")
    ...
    st.download_button(
        label=f"📥 TẢI XUỐNG TẤT CẢ ({st.session_state.success_count} LINK THÀNH CÔNG - FILE .ZIP)",
